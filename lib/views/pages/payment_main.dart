...
class PaymentMain extends StatelessWidget {
  Function onEdit;
  PaymentMain({required this.onEdit, required this.items, super.key});
  ...
  Widget getTitle() {
    return Helper.getText(
        content: "Payments", fontSize: 21, fontWeight: FontWeight.bold);
  }
  ...
  List<Widget> getItemsWidgets(BuildContext context) {
    ...
    for (Map item in items) {
      item["onTap"] = () {
        print("edit");
        onEdit(EditPaymentFormController(payment: item));
